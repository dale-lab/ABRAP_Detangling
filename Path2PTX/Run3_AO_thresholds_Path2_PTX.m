function [structure] = ABRAP_thresholds_Path2_PTX(parameters, data, dataTimes)
...
NUMindex = ceil(NUMindex);
THRESHhighmoave(1:NUMindex) = HIGHval;
THRESHlowmoave(1:NUMindex) = LOWval;


%complete moving average of high and k
for u = ceil(NUMindex+1):ceil(numel(dataTimes(:))-NUMindex)-1
    AVEhigh = AVEhigh - THRESHhighs(ceil(u-NUMindex)) + THRESHhighs(ceil(u+NUMindex));
    AVElow = AVElow - THRESHlows(ceil(u-NUMindex)) + THRESHlows(ceil(u+NUMindex));  
    HIGHval = AVEhigh/TIMEave;
    LOWval = AVElow/TIMEave;
    THRESHhighmoave(u) = HIGHval;
    THRESHlowmoave(u) = LOWval;
end
...
THRESHhighmoave(end-NUMindex:end) = HIGHval;
THRESHlowmoave(end-NUMindex:end) = LOWval;
