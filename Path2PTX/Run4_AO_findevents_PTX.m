function [characteristics] = ABRAP_findevents_PTX(stim, parameters, structure, OOP)
...
if OOP == 1
    ...
    for j = 1:numel(STARTtimes)
        ...
        for k = ceil(FREQUENCY*thirdlength):ceil(FREQUENCY*ENDtimes(j)-(ENDtimes(j) - STARTtimes(j)))
            if stim(k) < 0.01 && stim(k+1) >= 0.01
                stimcount = stimcount + 1;
            end
        end
        density = stimcount/(breathlength/3);
        if density >= 200
            STARTtimes(j) = -1000;
            ENDtimes(j) = -1000;
        end
